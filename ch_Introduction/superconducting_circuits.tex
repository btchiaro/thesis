...
It is apparent that a terminated segment of lossy transmission line should behave like a damped parallel RLC oscillator by inspecting the input impedance of such a line \cite{Pozar}
\qeqn{
...
This permits rapid fabrication iterations and protects against performance degradation that sometimes results from complex multi-step fabrication sequence.
...
\quickfig{1 in}{./PDF/LCO.pdf}{This is a simple LC oscillator (resonator) with position and momentum coordinates $\Phi$ and $Q$}{LCO}
...
This challenge is addressed by using the nonlinear inductance of a josephson junction
...
The term $h_i n_i$ can be understood as the energy of an $LC$ harmonic oscillator with frequency $h_i$.
...
It should be noted that this can be thought of as a single term in an expansion, it gives $0$ for $n_i = 1, 2$ and $U$ for $n=2$.
