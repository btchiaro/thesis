...
between qubits.
Using the Harmonic oscillator relations in eqns~\ref{ho_cre_op} and \ref{ho_ann_op}
%\qeqn{
...
The resulting device can be thought of as a Josephson junction with a large shunting capacitance.
This makes the transmon a weakly anharmonic LC oscillator.
The idea behind the transmon is that the shunting capacitance \verify{exponentially} surpresses charge noise by trading away anharmonicity\cite{Koch2007}.
...
It should be noted that this can be though of as a single term in an expansion, it gives $0$ for $n_i = 1, 2$ and $U$ for $n=2$.
